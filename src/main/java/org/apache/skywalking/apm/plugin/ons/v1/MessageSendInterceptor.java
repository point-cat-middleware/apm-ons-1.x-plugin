...
public class MessageSendInterceptor implements InstanceMethodsAroundInterceptor {
    ...
    public void beforeMethod(EnhancedInstance objInst, Method method, Object[] allArguments, Class<?>[] argumentsTypes,
        ...
        if (StringUtil.isNotBlank(topic) && topic.startsWith(Constants.FILTER_TOPIC_PREFIX)) {
            if (allArguments[6] != null) {
                ((EnhancedInstance) allArguments[6]).setSkyWalkingDynamicField(new SendCallBackEnhanceInfo(topic, null));
            }
            return;
        }
        ...
        AbstractSpan span = ContextManager.createExitSpan(buildOperationName(topic), contextCarrier, namingServiceAddress);
        span.setComponent(ComponentsDefine.ROCKET_MQ_PRODUCER);
        Tags.MQ_BROKER.set(span, (String) allArguments[0]);
        Tags.MQ_TOPIC.set(span, topic);
        ...
        if (allArguments[6] != null) {
            ((EnhancedInstance) allArguments[6]).setSkyWalkingDynamicField(new SendCallBackEnhanceInfo(topic, ContextManager
                .capture()));
        }
    }
